...
<x-app-layout>
    ...
    <form method="post" id="cargo_handling_setting_form" action="{{ route('customer_cargo_handling_setting.update') }}" class="m-0">
        ...
        <div class="py-5 mx-5 grid grid-cols-12">
            <table class="col-span-10 text-sm mb-5">
                <thead>
                    <tr class="font-normal text-left text-white bg-gray-600 border-gray-600 sticky top-0">
                        <th class="p-2 px-2 w-2/12">荷役名</th>
                        <th class="p-2 px-2 w-4/12">荷役備考</th>
                        <th class="p-2 px-2 w-2/12 text-right">荷役単価</th>
                        <th class="p-2 px-2 w-2/12 text-center">収支登録初期表示</th>
                ...
                <tbody id="cargo_handling_setting_body" class="bg-white">
                    @foreach($cargo_handling_settings as $cargo_handling_setting)
                        <tr id="tr_{{ $cargo_handling_setting->cargo_handling_id.'-'.$cargo_handling_setting->cargo_handling_unit_price.'-'.$cargo_handling_setting->cargo_handling_note.'-'.$cargo_handling_setting->cargo_handling_customer_id.'-kizon' }}">
                            <td class="p-1 px-2 border">{{ $cargo_handling_setting->cargo_handling->cargo_handling_name }}</td>
                            <td class="p-1 px-2 border"><input type="text" name="cargo_handling_note[{{ $cargo_handling_setting->cargo_handling_id.'-'.$cargo_handling_setting->cargo_handling_unit_price.'-'.$cargo_handling_setting->cargo_handling_note.'-'.$cargo_handling_setting->cargo_handling_customer_id.'-kizon' }}]" class="text-sm w-full bg-gray-100 cargo_handling_note" value="{{ $cargo_handling_setting->cargo_handling_note }}" autocomplete="off"></td>
                            <td class="p-1 px-2 border text-right"><input type="tel" name="cargo_handling_unit_price[{{ $cargo_handling_setting->cargo_handling_id.'-'.$cargo_handling_setting->cargo_handling_unit_price.'-'.$cargo_handling_setting->cargo_handling_note.'-'.$cargo_handling_setting->cargo_handling_customer_id.'-kizon' }}]" class="text-sm text-right w-3/4 bg-gray-100 cargo_handling_unit_price" value="{{ $cargo_handling_setting->cargo_handling_unit_price }}" autocomplete="off"><span class="mt-5 ml-1">円</span></td>
                            <td class="p-1 px-2 border text-center"><input type="checkbox" name="balance_register_default_disp[{{ $cargo_handling_setting->cargo_handling_id.'-'.$cargo_handling_setting->cargo_handling_unit_price.'-'.$cargo_handling_setting->cargo_handling_note.'-'.$cargo_handling_setting->cargo_handling_customer_id.'-kizon' }}]" class="text-sm bg-gray-100" {{ $cargo_handling_setting->balance_register_default_disp == 1 ? 'checked' : '' }}></td>
                            <td class="p-1 px-2 border text-center">
                                @if(Auth::user()->role_id == 1 || Auth::user()->role_id == 11 && Auth::user()->base_id == $customer->control_base_id)
                                    <button type="button" id="{{ $cargo_handling_setting->cargo_handling_id.'-'.$cargo_handling_setting->cargo_handling_unit_price.'-'.$cargo_handling_setting->cargo_handling_note.'-'.$cargo_handling_setting->cargo_handling_customer_id.'-kizon' }}" class="cargo_handling_setting_delete bg-red-600 text-white hover:bg-gray-400 p-1 text-xs">削除</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
