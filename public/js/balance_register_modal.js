...
(() => {
/*!************************************************!*\
  !*** ./resources/js/balance_register_modal.js ***!
  \************************************************/
...
var other_expense_select = document.getElementById('other_expense_select'); // その他経費選択のセレクトボックス

var other_expense_list = document.getElementById('other_expense_list'); // 要素追加先のDivタグ

var total_other_expense_div = document.getElementById('total_other_expense_div'); // その他経費の合計divタグ

var total_other_expense_amount = document.getElementById('total_other_expense_amount'); // その他経費金額合計のpタグ
...
$("[id=shipping_method_add]").on("click", function () {
  ...
  try {
    ...
    var select_fare_expenses = select_value_split[2].replace('円)', ''); // 売上の運賃情報を追加

    fare_add('sales', select_id, select_shipping_method, select_fare_unit_price); // 経費の運賃情報を追加
...
function fare_add(add_category, select_id, select_shipping_method, select_price) {
  ...
  box_quantity.classList.add('text-sm', 'col-span-1', 'text-right', 'fare_amount_' + add_category + '_update', 'h-4/5', 'box_quantity_' + add_category); // 個口数という文字を表示する要素を作成
  ...
  fare_unit_price.classList.add('text-sm', 'col-span-1', 'text-right', 'fare_amount_' + add_category + '_update', 'h-4/5'); // 円という文字を表示する要素を作成
  ...
  fare_amount.classList.add('text-sm', 'col-span-2', 'text-right', 'h-4/5', 'fare_amount_' + add_category); // 削除ボタンの要素を作成
...
$("[id=customer_select]").on("change", function () {
  // 荷主IDを取得
  var customer_id = customer_select.value;
  $.ajax({
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    url: '/balance_register_customer_data_get_ajax/' + customer_id,
    type: 'GET',
    dataType: 'json',
...
$("[id=cargo_handling_add]").on("click", function () {
  ...
  try {
    ...
    var cargo_handling_unit_price_value = select_value_split[1].replace('円)', ''); // 荷役要素を追加
...
function cargo_handling_add(add_id, cargo_handling_name_value, cargo_handling_unit_price_value) {
  ...
  operation_quantity.classList.add('text-sm', 'col-span-1', 'text-right', 'cargo_handling_amount_update', 'h-4/5', 'operation_quantity'); // 作業という文字を表示する要素を作成
  ...
  cargo_handling_unit_price.classList.add('text-sm', 'col-span-1', 'text-right', 'cargo_handling_amount_update', 'h-4/5'); // 円という文字を表示する要素を作成
  ...
  cargo_handling_amount.classList.add('text-sm', 'col-span-2', 'text-right', 'h-4/5', 'cargo_handling_amount'); // 削除ボタンの要素を作成
...
$("[id=other_expense_add]").on("click", function () {
  // 選択した荷役のidとなる値
  var select_id = other_expense_select.value; // 選択しているインデックス番号を取得

  var select_index = other_expense_select.selectedIndex; // 選択した経費の値を取得

  var select_value = other_expense_select.options[select_index].innerHTML;
  ...
  try {
    ...
    var other_expense_name = document.createElement('input');
    ...
    other_expense_name.name = 'other_expense_name[]'; // 経費金額を入力する要素を作成

    var other_expense_amount = document.createElement('input');
    ...
    other_expense_amount.classList.add('text-sm', 'col-span-2', 'col-start-9', 'text-right', 'other_expense_amount_update', 'h-4/5', 'other_expense_amount'); // 円という文字を表示する要素を作成

    var other_expense_amount_text = document.createElement('p');
    other_expense_amount_text.classList.add('text-sm', 'col-span-1', 'col-start-11', 'text-left', 'pt-5', 'ml-2');
    other_expense_amount_text.innerHTML = '円'; // 削除ボタンの要素を作成

    var delete_btn = document.createElement('button');
    delete_btn.type = 'button';
    delete_btn.id = select_id + '_other_expense_delete_btn';
    delete_btn.innerHTML = '<i class="las la-trash la-lg"></i>';
    delete_btn.classList.add('col-span-1', 'col-start-12', 'bg-red-600', 'text-white', 'hover:bg-gray-400', 'delete_other_expense', 'h-4/5'); // 追加する要素を纏めるdivタグを作成

    var target_div = document.createElement('div');
    target_div.id = select_id + '_other_expense_div';
    target_div.classList.add('grid', 'grid-cols-12', 'col-span-12', 'border-b-2', 'border-black', 'pt-2'); // divタグに作成した要素を追加

    target_div.append(other_expense_name, other_expense_amount, other_expense_amount_text, delete_btn); // 纏めたdivタグを追加
...
$(document).on("click", ".delete_other_expense", function () {
  var target_id = id_get($(this));
  var delete_target_div = document.getElementById(target_id + '_other_expense_div');
  delete_target_div.remove();
  total_other_expense_update();
});
...
function total_other_expense_update() {
  // 金額合計を更新
  var other_expense_amount_sum = 0;
  var other_expense_amount_element = document.getElementsByClassName('other_expense_amount');

  for (var i = 0; i < other_expense_amount_element.length; i++) {
