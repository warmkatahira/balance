...
$("[id=customer_register_modal_open]").on("click", function () {
  var modal = document.getElementById('customer_register_modal');
...
$("[id=customer_base_info_save]").on("click", function () {
  var customer_base_info_form = document.getElementById('customer_base_info_form');
  customer_base_validation(customer_base_info_form, '設定を保存しますか？');
}); // バリデーションと処理の実行確認

function customer_base_validation(form, msg) {
  var base_select = document.getElementById('base_id');
  var customer_name = document.getElementById('customer_name');
  var monthly_storage_fee = document.getElementById('monthly_storage_fee');
  var working_days = document.getElementById('working_days');
...
$("[id=cargo_handling_register]").on("click", function () {
  // 登録情報の要素を取得
  var cargo_handling_select = document.getElementById('cargo_handling_id');
  var cargo_handling_unit_price = document.getElementById('cargo_handling_unit_price');
  var balance_register_default_disp = document.getElementById('balance_register_default_disp');
  var cargo_handling_note = document.getElementById('cargo_handling_note'); // 作成した要素の追加先を取得

  var cargo_handling_setting_body = document.getElementById('cargo_handling_setting_body');
  ...
  try {
    ...
    var cargo_handling_tr = document.createElement('tr');
    cargo_handling_tr.id = 'tr_' + cargo_handling_select.value + '-' + cargo_handling_unit_price.value + '-' + cargo_handling_note.value + '-0-add'; // 荷役名のtdタグを作成

    var cargo_handling_name_td = document.createElement('td');
    cargo_handling_name_td.innerHTML = cargo_handling_select.options[cargo_handling_select.selectedIndex].innerHTML;
    cargo_handling_name_td.classList.add('p-1', 'px-2', 'border'); // 荷役備考のtdタグを作成
    ...
    cargo_handling_note_input.value = cargo_handling_note.value; // 荷役備考のtdタグにinputタグを追加

    cargo_handling_note_td.append(cargo_handling_note_input); // 荷役単価のtdタグを作成

    var cargo_handling_unit_price_td = document.createElement('td');
    cargo_handling_unit_price_td.classList.add('p-1', 'px-2', 'border', 'text-right'); // 荷役単価のinputタグを作成

    var cargo_handling_unit_price_input = document.createElement('input');
    ...
    cargo_handling_unit_price_input.readOnly; // spanタグを作成

    var cargo_handling_unit_price_span = document.createElement('span');
    cargo_handling_unit_price_span.classList.add('mt-5', 'ml-1');
    cargo_handling_unit_price_span.innerHTML = '円'; // 荷役単価のtdタグにinputタグを追加

    cargo_handling_unit_price_td.append(cargo_handling_unit_price_input, cargo_handling_unit_price_span); // 収支登録初期表示のtdタグを作成

    var balance_register_default_disp_td = document.createElement('td');
    balance_register_default_disp_td.classList.add('p-1', 'px-2', 'border', 'text-center'); // 収支登録初期表示のinputタグを作成

    var balance_register_default_disp_input = document.createElement('input');
    ...
    balance_register_default_disp_input.checked = balance_register_default_disp.checked; // 収支登録初期表示のtdタグにinputタグを追加

    balance_register_default_disp_td.append(balance_register_default_disp_input); // 削除のtdタグを作成
    ...
    var cargo_handling_delete_btn = document.createElement('button');
    ...
    cargo_handling_delete_btn.innerHTML = '削除'; // 削除のtdタグにbuttonタグを追加

    cargo_handling_delete_btn_td.append(cargo_handling_delete_btn); // 全てのタグをtrタグに追加

    cargo_handling_tr.append(cargo_handling_name_td, cargo_handling_note_td, cargo_handling_unit_price_td, balance_register_default_disp_td, cargo_handling_delete_btn_td); // trタグをtbodyタグに追加

    cargo_handling_setting_body.append(cargo_handling_tr); // change_flgをtrueに変更
...
$("[id=cargo_handling_setting_save]").on("click", function () {
  var cargo_handling_setting_form = document.getElementById('cargo_handling_setting_form');

  try {
    // 荷役単価を取得して、正常な値であるかチェック
    var cargo_handling_unit_prices = document.querySelectorAll('.cargo_handling_unit_price');
    cargo_handling_unit_prices.forEach(function (cargo_handling_unit_price) {
      // 荷役単価が正しいかチェック
      if (!cargo_handling_unit_price.value || isNaN(cargo_handling_unit_price.value)) {
        throw new Error('荷役単価が正しくありません。');
      }
...
$(document).on("click", ".cargo_handling_setting_delete", function () {
  // DOM要素を削除
  var delete_target_id = $(this).attr('id');
  var delete_target = document.getElementById('tr_' + delete_target_id);
  delete_target.remove(); // 削除対象のidをスプリット

  var split_id = delete_target_id.split('-'); // 荷役単価のinputタグを作成

  var cargo_handling_unit_price_input = document.createElement('input');
  cargo_handling_unit_price_input.type = 'hidden';
  cargo_handling_unit_price_input.name = 'cargo_handling_unit_price[---' + split_id[3] + '-del' + ']'; // inputタグをtbodyタグに追加

  cargo_handling_setting_body.append(cargo_handling_unit_price_input); // change_flgをtrueに変更

  change_flg = true;
}); // 配送方法設定登録モーダルを開く

$("[id=shipping_method_register_modal_open]").on("click", function () {
  var modal = document.getElementById('shipping_method_register_modal');
  modal.classList.remove('hidden');
}); // 配送方法設定登録モーダルを閉じる
...
$("[id=shipping_method_register]").on("click", function () {
  // 登録情報の要素を取得
  var shipping_method_select = document.getElementById('shipping_method_id');
  var shipping_method_note = document.getElementById('shipping_method_note');
  var fare_unit_price = document.getElementById('fare_unit_price');
  var fare_expense = document.getElementById('fare_expense'); // 作成した要素の追加先を取得

  var shipping_method_setting_body = document.getElementById('shipping_method_setting_body');
  ...
  try {
    ...
    var select_value = shipping_method_select.options[shipping_method_select.selectedIndex].innerHTML.split('【');
    var shipping_company = select_value[0];
    var shipping_method = select_value[1].replace('】', ''); // trタグを作成

    var shipping_method_tr = document.createElement('tr');
    shipping_method_tr.id = 'tr_' + shipping_method_select.value + '-' + shipping_method_note.value + '-' + fare_unit_price.value + '-' + fare_expense.value + '-0-add'; // 運送会社のtdタグを作成

    var shipping_company_td = document.createElement('td');
    shipping_company_td.innerHTML = shipping_company;
    shipping_company_td.classList.add('p-1', 'px-2', 'border'); // 配送方法のtdタグを作成

    var shipping_method_td = document.createElement('td');
    shipping_method_td.innerHTML = shipping_method;
    shipping_method_td.classList.add('p-1', 'px-2', 'border'); // 配送方法備考のtdタグを作成

    var shipping_method_note_td = document.createElement('td');
    shipping_method_note_td.classList.add('p-1', 'px-2', 'border', 'text-center'); // 配送方法備考のinputタグを作成

    var shipping_method_note_input = document.createElement('input');
    ...
    shipping_method_note_input.value = shipping_method_note.value; // 配送方法備考のtdタグにinputタグを追加

    shipping_method_note_td.append(shipping_method_note_input); // 運賃単価のtdタグを作成
    ...
    var shipping_method_delete_btn = document.createElement('button');
    ...
    shipping_method_delete_btn.innerHTML = '削除'; // 削除のtdタグにbuttonタグを追加

    shipping_method_delete_btn_td.append(shipping_method_delete_btn); // 全てのタグをtrタグに追加

    shipping_method_tr.append(shipping_company_td, shipping_method_td, shipping_method_note_td, fare_unit_price_td, fare_expense_td, shipping_method_delete_btn_td); // trタグをtbodyタグに追加

    shipping_method_setting_body.append(shipping_method_tr); // change_flgをtrueに変更
...
$(document).on("click", ".shipping_method_setting_delete", function () {
  ...
  var split_id = delete_target_id.split('-'); // 運賃単価のinputタグを作成

  var fare_unit_price_input = document.createElement('input');
  fare_unit_price_input.type = 'hidden';
  fare_unit_price_input.name = 'fare_unit_price[----' + split_id[4] + '-del' + ']'; // inputタグをtbodyタグに追加

  shipping_method_setting_body.append(fare_unit_price_input); // change_flgをtrueに変更

  change_flg = true;
}); // 配送方法設定保存ボタンが押下されたら

$("[id=shipping_method_setting_save]").on("click", function () {
  var shipping_method_setting_form = document.getElementById('shipping_method_setting_form');

  try {
    // 運賃単価を取得して、正常な値であるかチェック
    var fare_unit_prices = document.querySelectorAll('.fare_unit_price');
    fare_unit_prices.forEach(function (fare_unit_price) {
      // 運賃単価が正しいかチェック
      if (!fare_unit_price.value || isNaN(fare_unit_price.value)) {
        throw new Error('運賃単価が正しくありません。');
      }
    ...
    var fare_expenses = document.querySelectorAll('.fare_expense');
    fare_expenses.forEach(function (fare_expense) {
      // 経費単価が正しいかチェック
      if (!fare_expense.value || isNaN(fare_expense.value)) {
        throw new Error('経費単価が正しくありません。');
      }
