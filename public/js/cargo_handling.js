...
$("[id=cargo_handling_register]").on("click", function () {
  ...
  try {
    ...
    cargo_handling_tr.id = 'tr_' + cargo_handling_name.value; // 荷役名のtdタグを作成

    var cargo_handling_name_td = document.createElement('td');
    cargo_handling_name_td.classList.add('p-1', 'px-2', 'border'); // 荷役名のinputタグを作成

    var cargo_handling_name_input = document.createElement('input');
...
$("[id=cargo_handling_save]").on("click", function () {
  var cargo_handling_form = document.getElementById('cargo_handling_form');

  try {
    var result = window.confirm('荷役マスタを保存しますか？'); // 「はい」が押下されたらsubmit、「いいえ」が押下されたら処理キャンセル
